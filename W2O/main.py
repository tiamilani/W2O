...
from W2O.src.util.strings import s
from W2O.src.util.checks import check_conf as cc
from W2O.src.io.file import FileHandler as fh
...
parser.add_argument("--oral", dest="oral", default="ListaStudenti_appello_registrazione.xls",
                    action="store", help="defines the name of the xls file that represent the list of the students enroled for the oral exam")
...
parser.add_argument("-o", "--output", dest="output", default="result/schedule.xls",
...
def main():
    ...
    fh_oral = fh(options.oral, override=False, already_exists=True)
    ...
    fh_output = fh(options.output, override=True, already_exists=False)

    students = pd.read_excel(fh_students.path, skiprows=data[s.skyp_row][s.vl],
                             index_col=s.students_idx)
    oral = pd.read_excel(fh_oral.path, skiprows=data[s.skyp_row][s.vl],
                         index_col=s.students_idx)
    ...
    preamble_oral = pd.read_excel(fh_oral.path, nrows=data[s.skyp_row][s.vl]-1)
    ...
    for matricola, grade in zip(grades["Matricola"].values, grades["Valutazione/11,00"].values):
        ...
        if matricola not in oral.index:
            print("WARNING! the student {}, {} {},\tgrade: {},\t".format(
                    matricola, students.loc[[matricola]]["Cognome"].values[0],
                    students.loc[[matricola]]["Nome"].values[0], grade
                    ) +
                  "is not registered for the oral exam")
        if correct_grade >= data[s.s_threshold][s.vl]:
            ...
            oral.loc[matricola, "Esito"] = str(grade).replace(grade_separetor, options.separetor)
            oral.loc[matricola, "Esito_round"] = round(correct_grade/11*30)
            number_of_passed += 1
        else:
            students.loc[matricola, "Esito"] = "0"
            students.loc[matricola, "Esito_round"] = "0"
            oral.loc[matricola, "Esito"] = "0"
            oral.loc[matricola, "Esito_round"] = "0"
    ...
    oral = oral.reset_index()
    oral = oral.set_index('#')
    oral_round = oral[s.STUDENTS_COLUMNS_ROUND]
    oral = oral[s.STUDENTS_COLUMNS]
    oral_round.rename(columns={'Esito_round': 'Esito'}, inplace=True)

    oral.index = oral.index.map(str)
    oral = oral.astype({'Matricola': str, 'Unnamed: 6': str})
    preamble_oral = preamble_oral.fillna('')
    preamble_oral = preamble_oral.astype(str)

    oral.rename(columns={'Unnamed: 6': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 5': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 6': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 7': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 8': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 9': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 10': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 11': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 12': ''}, inplace=True)
    preamble_oral.rename(columns={'Unnamed: 13': ''}, inplace=True)

    writer = pd.ExcelWriter(data[s.output_folder][s.vl].path + "/" + fh_oral.fileName)
    pandas.io.formats.excel.ExcelFormatter.header_style = None
    preamble_oral.to_excel(writer, sheet_name='esse3', index=False)
    oral.to_excel(writer, startrow=data[s.skyp_row][s.vl], sheet_name='esse3')
    writer.save()

    if options.round:
        file_name = fh_oral.fileName.split('.')[0] + "_rounded.xls"
        writer = pd.ExcelWriter(data[s.output_folder][s.vl].path + "/" + file_name)
        pandas.io.formats.excel.ExcelFormatter.header_style = None
        preamble_oral.to_excel(writer, sheet_name='esse3', index=False)
        oral_round.to_excel(writer, startrow=data[s.skyp_row][s.vl], sheet_name='esse3')
        writer.save()
